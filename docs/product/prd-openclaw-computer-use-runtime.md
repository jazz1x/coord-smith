...
- `scope_ceiling` must equal `runCompletion` (the current released ceiling).
  `pageReadyObserved` is accepted for transitional compatibility (the adapter
  in `src/ez_ax/graph/released_call_site.py` recognizes both values), but new
  callers should send `runCompletion`; see `docs/prd.md` as Source-Of-Truth.
...
- adapter request envelope field -> MCP tool input field:
  ...
  - `scope_ceiling` -> `scope_ceiling` (string, must equal `runCompletion`;
    `pageReadyObserved` still accepted for transitional compatibility)
