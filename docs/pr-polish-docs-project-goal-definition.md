...
coord-smith production hardening: MCP 스캐폴드 영구 제거 · YAML 레시피 지원 · 12 노드 증거 priority 게이트 · async correctness · doc 컨벤션 정리.

## 왜 이 변경이 필요한가 (배경)

coord-smith의 단일 transport은 CLI subprocess (`coord-smith --click-recipe`)이지만, 사용된 적 없는 MCP 스캐폴드 코드가 `src/`에 남아 dead-dependency(`mcp` package)와 의도-구현 혼선을 만들었다.
...
| 영역 | 변경 |
|------|------|
| 코드 — MCP 제거 | [src/coord_smith/config/mcp_stdio.py / mcp_stdio_cli.py](src/coord_smith/config/) 삭제, langchain context 삭제, `mcp` deps 제거 |
| 코드 — YAML 레시피 | [click_recipe.py](src/coord_smith/config/click_recipe.py) `.yaml`/`.yml` 확장자 자동 라우팅, JSON은 backwards-compat |
| 코드 — 증거 게이트 | [evidence/envelope.py](src/coord_smith/evidence/envelope.py)의 `enforce_evidence_priority_gate()` 신설, [released_call_site.py](src/coord_smith/graph/released_call_site.py) 12 노드 전부 적용 |
| 코드 — async 정합 | [pyautogui_adapter.py](src/coord_smith/adapters/pyautogui_adapter.py)의 `_verified_click` / `preflight` / `_await_post_click_signal` / `wait_for_image` 모두 `async` 전환, `time.sleep` → `asyncio.sleep` |
| 코드 — 검증 | [execution/client.py](src/coord_smith/adapters/execution/client.py)의 `ExecutionResult.__post_init__` validation, ghost evidence ref(`evidence://text/fallback-reason`) 12개 미션에서 일괄 제거, `parse_released_evidence_ref` 중복 호출부 통합 |
...
| 파일 그룹 | 변경 |
|-----------|------|
| `src/coord_smith/config/` | MCP 모듈 삭제, YAML 레시피 라우팅 |
| `src/coord_smith/adapters/` | langchain/ 삭제, pyautogui_adapter async 전환·중복 제거, execution/client.py validation |
| `src/coord_smith/evidence/` | priority gate 신설 |
| `src/coord_smith/graph/` | 12 노드에 gate 적용, base_dir 와이어링 수정 |
