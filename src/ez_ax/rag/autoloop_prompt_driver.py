...
def _continuation_seed_prompt(*, next_action: str) -> str:
    return (
        "Use $ez-ax-low-attention-autoloop, but do not stop just because "
        "docs/product/work-rag.json says FINAL_STOP. First run the documented "
        "stop-state consistency gate across docs/current-state.md, "
        ...
        "adjacent-surface completion check. If that gate still exhausts "
        "honestly, switch to continuation-seeding mode for exactly one "
        "one-commit-safe task: either add one explicit PRD-backed queue item "
        "for an omitted in-bounds surface, or add one docs-sufficiency "
        "improvement that makes the next lower-capacity implementation slice "
